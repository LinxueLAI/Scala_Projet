  ...
  */
  def main(args: Array[String]): Unit = {
    ...
    Traverse all the numbers from 1 to one million, and calculate the length of each chain produced by iterative rules :
                                n → n/2 (n is even)
                                n → 3n + 1 (n is odd)
    Used a sequence to save all the iterative elements -> def produceChain(n:Int):Seq[Long] = {...}

    Advantage : algorithm is easy to understand

    Disadvantage : high time complexity :74707 ms = 75s = more than 1min
     */
